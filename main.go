...
func Linear_interp(start, end []float64, steps int) [][]float64 {
	...
	interp[0] = start                    // we dont need to set end as it will be calculated.
	interp[steps+1] = end
...
// varialbes are the labeled corners as b= 0 , t= 1
// bb bt
// tb tt
//
...
func Bilinear_interp(bb, bt, tb, tt []float64, steps int) [][][]float64 {
	...
	// initialize -- require unfortunately
...
func main() {
	...
	cube := Trilinear_interp(corners[0], corners[1], corners[2], corners[3], corners[4], corners[5], corners[6], corners[7], 200)
	// for i, slice := range cube {
